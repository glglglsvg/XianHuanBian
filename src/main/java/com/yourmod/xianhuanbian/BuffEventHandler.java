...
public class BuffEventHandler {
    ...
    private static double getBaseAttack(int id) {
        return switch(id) {
            case 1 -> 2; case 2 -> 4; case 3 -> 6; case 4 -> 8;
            case 5 -> 10; case 6 -> 12; case 7 -> 14; case 8 -> 16;
            case 9 -> 18; default -> 0;
        };
    }
    ...
    private static void applySingleBuff(ServerPlayerEntity p, int id, PlayerBuffData d) {
        ...
        if(attr!=null && id <= 9) {
            double total = getBaseAttack(id) * Math.pow(2, lv);
    ...
    public static void processActivity(ServerPlayerEntity p, PlayerBuffData d, float inc) {
        ...
        for(int i=0;i<10;i++) {
            if(!d.isUnlocked(i+1)) {
                ...
                if(p.getRandom().nextFloat() < d.getChance(i)) {
                    d.setUnlocked(i+1,true); d.setActive(i+1,true);
        ...
        if (!all10) {
            ...
            if (!candidates.isEmpty()) {
                ...
                float upChance = d.hasBehaviorUpgraded(id) ? 0.01f : 0.5f;
                if (p.getRandom().nextFloat() < upChance) {
        ...
        if(all10 && !d.isUnlocked(11)) { d.setUnlocked(11,true); p.sendMessage(net.minecraft.text.Text.literal("仙环归一，拾壹·仙变已解锁！"),false); }
        if(all10 && d.isUnlocked(11) && !d.isUnlocked(12)) {
            boolean allActive = true;
            for(int i=1;i<=11;i++) if(!d.isActive(i)) allActive=false;
            if(allActive) { d.setUnlocked(12,true); p.sendMessage(net.minecraft.text.Text.literal("不灭之环已降下！"),false); }
        }
    }
    ...
    public static void handleKillExperience(ServerPlayerEntity p, PlayerBuffData d, LivingEntity t) {
        ...
        while(d.getExpPool() >= d.getNextThreshold()) {
            ...
            d.setBonusAttack(d.getBonusAttack()+2); d.setBonusHealth(d.getBonusHealth()+2);
